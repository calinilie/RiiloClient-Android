...
public class WorkerService extends IntentService{
	...
	protected void onHandleIntent(Intent intent) {
		...
		case StringKeys.WS_INTENT_GET_LATEST_POSTS:
			...
			//send them new posts to the UI
			resultData = new Bundle();
			resultData.putParcelable(StringKeys.POST_LIST_PARCELABLE, new PostsListParcelable(newPosts));
			resultReceiver.send(StringKeys.POST_RESULT_RECEIVER_CODE_LATEST_POSTS, resultData);
			break;
		case StringKeys.WS_INTENT_GET_CONVERSATION_FROM_CONVERSATION_ID:
			...
			resultData = new Bundle();
			resultData.putParcelable(StringKeys.POST_LIST_PARCELABLE, new PostsListParcelable(newPostsInConversation));
		...
		case StringKeys.WS_INTENT_GET_NOTIFICATIONS:
			...
			List<Post> newNotifications = postsCache.addPostsToNotifications(notifications);
			
			if (resultReceiver!=null){
				resultData = new Bundle();
				resultData.putParcelable(StringKeys.POST_LIST_PARCELABLE, new PostsListParcelable(newNotifications));
				resultData.putInt(StringKeys.POST_RESULT_RECEIVER_NOTIFICATION_NUMBER, newNotifications.size());
				resultReceiver.send(StringKeys.POST_RESULT_RECEIVER_CODE_NOTIFICATIONS, resultData);
			}
			break;
		...
		case StringKeys.WS_INTENT_NEARBY_POSTS:
			...
			//fire HTTP request to get new nearby Posts
			if (latitude != 0 && longitude != 0){
				nearbyPosts = getNearbyPosts(latitude, longitude);
			}
			
			List<Post> newNearbyPosts = postsCache.addPostsToNearby(nearbyPosts);
			
			if (resultReceiver!=null){
				resultData = new Bundle();
				resultData.putParcelable(StringKeys.POST_LIST_PARCELABLE, new PostsListParcelable(newNearbyPosts));
				resultReceiver.send(StringKeys.POST_RESULT_RECEIVER_CODE_NEARBY_POSYS, resultData);
			}
			break;
...
//	                sendNotification("Deleted messages on server: " + extras.toString());
	...
	private List<Post> getLatestPostsRequest(int start, int limit){
		//build endpoint
